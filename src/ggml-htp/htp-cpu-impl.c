...
static struct ggml_arm_arch_features_type ggml_arm_arch_features = {-1, -1, -1, -1, 0};
#endif
...
// __attribute__((weak)) const struct ggml_type_traits_cpu * ggml_get_type_traits_cpu(enum ggml_type type) {
...
// NOTE(hzx): code here is superseded by the newer implementation in ggml-cpu
/*
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                if (htp_ops_support_op(tensor)) {
                    ...
                    if (params->ith == 0) {
                        buf = ggml_aligned_malloc(n);
                        GGML_ASSERT(buf);

                        asm volatile ("dmb sy":::"memory");
                        ggml_backend_tensor_get(tensor, buf, 0, n);
                        memset(tensor->data, 0, n);
                    }
                    ggml_barrier_htp(params->threadpool);
                    ...
                    if (params->ith == 0) {
                        const size_t head_dim = tensor->ne[0];
                        for (int i = 0; i < tensor->ne[2]; ++i) { // n_tokens
                            if (i > 1) break;

                            for (int h = 0; h < tensor->ne[1]; ++h) { // n_heads
                                ssize_t offset = i * tensor->nb[2] + h * tensor->nb[1];
                                float *x = (float *)(buf + offset);
                                float *y = (float *)((uint8_t *)tensor->data + offset);
                                ...
                                float rmse = sqrtf(s / head_dim);
                                fprintf(stderr, "!! RMSE for head %d, token %d: %f\n", h, i, rmse);
                                fprintf(stderr, "    sampled x: %f %f %f %f ...\n", x[0], x[1], x[2], x[3]);
                                fprintf(stderr, "    sampled y: %f %f %f %f ...\n", y[0], y[1], y[2], y[3]);
                            }
                        }

                        ggml_aligned_free(buf, n);
                    }
                    */
                } else {
                    ggml_compute_forward_flash_attn_ext(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], tensor);
...
// __attribute__((weak)) void ggml_threadpool_free(struct ggml_threadpool* threadpool) {
//     if (!threadpool) {
//         return;
//     }
...
// __attribute__((weak)) void ggml_threadpool_pause(struct ggml_threadpool * threadpool) {
//     ggml_mutex_lock(&threadpool->mutex);
//     if (!threadpool->pause) {
...
// __attribute__((weak)) void ggml_threadpool_resume(struct ggml_threadpool * threadpool) {
//     ggml_mutex_lock(&threadpool->mutex);
//     if (threadpool->pause) {
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        //     snprintf(path, sizeof(path), "htp-dump/%s.bin", node->name);
...
enum ggml_status ggml_graph_compute_htp_hybrid(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    if (threadpool == NULL) {
        ...
        // NOTE(hzx): we may set tpp.cpumask & tpp.strict_cpu here for thread affinity.
        // tpp.cpumask[2] = 1;
        // tpp.cpumask[3] = 1;
        // tpp.cpumask[4] = 1;
        // tpp.cpumask[5] = 1;
        ...
        threadpool = ggml_threadpool_new_impl(&tpp, cgraph, cplan);
    } else {
